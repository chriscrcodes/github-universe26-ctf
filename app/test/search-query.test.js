...
test("the price and name filters bind their values", () => {
  ...
  const cityFilter = buildCityFilter("Paris");
  assert.ok([
    JSON.stringify({ clause: "city = 'Paris' COLLATE NOCASE", parameters: [] }),
    JSON.stringify({ clause: "city = ? COLLATE NOCASE", parameters: ["Paris"] }),
  ].includes(JSON.stringify(cityFilter)), "support the baseline and the approved parameter-binding correction");
  assert.ok(query.sql.includes(cityFilter.clause));
